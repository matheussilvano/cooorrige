...
export default function ResultCard({ result, essayId, initialReview, onSaveReview }: ResultCardProps) {
  ...
  const hasOcrError = String(result?.status || "").toLowerCase() === "erro_ocr";
  const ocrMessage = result?.error_message || result?.detail || result?.message || "Não foi possível ler o arquivo enviado. Tente novamente com melhor qualidade.";

  if (hasOcrError) {
    return (
      <Card className="p-6">
        <h3 className="text-base font-semibold">Resultado</h3>
        <p className="mt-3 rounded-xl border border-rose-200 bg-rose-50 p-3 text-sm text-rose-700">{ocrMessage}</p>
        <p className="mt-3 text-xs text-text-muted">Nenhum crédito foi debitado nessa tentativa.</p>
      </Card>
    );
  }
